...
/** 코골이 이벤트 저장 컨트롤러
 *
 * 1.url params 에서 sessionId 추출
 * 2.body 에서 starTime, endTime, avgConfidence를 추출
 * 3. 값 검증 후 service에 저장 요청을 보낸다.
 */
const createSnoreEvent = async (req, res) => {
  try {
    // URL 들어온 SESSIONid 추출
    const { sessionId } = req.params;

    // 요청 body 에서 코골이 이벤트 정보 추출
    ...
    //존재 하지 않는 sessionId인 경우
    if (error.message === "세션을 찾을 수 없습니다.") {
      return res.status(404).json({
        success: false,
        data: {},
        message: "존재하지 않는 세션 입니다.",
      });
    }
    return res.status(500).json({
      success: false,
